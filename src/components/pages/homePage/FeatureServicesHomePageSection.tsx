    ...
    <section id="why" style={{ padding: "8rem 0", borderTop: "1px solid var(--glass-bg)" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 1.5rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "center" }}>
          ...
          <div>
            <div style={{ display: "inline-flex", fontFamily: "var(--font-mono, monospace)", fontSize: "0.65rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--accent-deep)", background: "rgba(108,99,255,0.07)", border: "1px solid var(--glass-bg)", padding: "0.3rem 0.8rem", borderRadius: 100, marginBottom: "1.25rem" }}>
              {eyebrow}
            </div>
            <h2 style={{ fontFamily: "var(--font-mono, monospace)", fontSize: "clamp(1.7rem,3.5vw,2.8rem)", fontWeight: 700, letterSpacing: "-0.025em", lineHeight: 1.1, color: "var(--text)", marginBottom: "2.5rem", whiteSpace: "pre-line" }}>
              {heading}
            </h2>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {items.map((it, i) => {
                ...
                >
                  ...
                  <div>
                    <div style={{ fontFamily: "var(--font-mono, monospace)", fontSize: "0.875rem", fontWeight: 700, color: "var(--text)", marginBottom: "0.3rem" }}>{it.title}</div>
          ...
          <div style={{ borderRadius: 24, background: "rgba(255,255,255,0.72)", border: "1px solid rgba(255,255,255,0.85)", backdropFilter: "blur(28px) saturate(200%)", boxShadow: "0 16px 56px rgba(99,88,210,0.18)", padding: "1.75rem", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ fontFamily: "var(--font-mono, monospace)", fontSize: "0.65rem", letterSpacing: "0.18em", textTransform: "uppercase", color: "var(--text-dim)", marginBottom: "1.25rem" }}>{previewLabel}</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", marginBottom: "1.25rem" }}>
              {stats.map((m, i) => (
                <div key={m.label} style={{ padding: "1rem", borderRadius: 14, background: "rgba(255,255,255,0.55)", border: "1px solid rgba(255,255,255,0.8)" }}>
                  <div style={{ fontFamily: "var(--font-mono, monospace)", fontSize: "1.4rem", fontWeight: 700, ...(i % 2 === 0 ? { background: "linear-gradient(135deg,var(--accent),var(--accent-2))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" } : { color: "var(--accent-teal)" }) }}>{m.val}</div>
                  <div style={{ fontFamily: "var(--font-mono, monospace)", fontSize: "0.68rem", color: "var(--text-dim)", letterSpacing: "0.08em", marginTop: "0.2rem" }}>{m.label}</div>
                </div>
              ))}
            </div>
            ...
            {bars.map((b, i) => (
              <div key={b.label} style={{ marginBottom: "0.9rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontFamily: "var(--font-mono, monospace)", fontSize: "0.65rem", color: "var(--text-muted)", marginBottom: "0.4rem", letterSpacing: "0.06em" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "1rem", fontFamily: "var(--font-mono, monospace)", fontSize: "0.65rem", color: "var(--text-dim)", letterSpacing: "0.08em" }}>
