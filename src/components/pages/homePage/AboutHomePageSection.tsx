  ...
  return (
    <section style={{ borderTop: "1px solid rgba(108,99,255,0.08)" }} className="py-20 md:py-32">
      <div className="grid grid-cols-1 md:grid-cols-2 items-center gap-10 md:gap-20" style={{ maxWidth: 1280, margin: "0 auto", padding: "0 1.25rem" }}>
        <div>
          <div style={{ display: "inline-flex", fontFamily: "var(--font-mono, monospace)", fontSize: "0.65rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "#6c63ff", background: "rgba(108,99,255,0.07)", border: "1px solid rgba(108,99,255,0.18)", padding: "0.3rem 0.8rem", borderRadius: 100, marginBottom: "1.25rem" }}>Our story</div>
        ...
        <div className="p-8 sm:p-10" style={{ borderRadius: 24, background: "rgba(255,255,255,0.55)", border: "1px solid rgba(255,255,255,0.85)", backdropFilter: "blur(20px)", boxShadow: "0 12px 48px rgba(99,88,210,0.14)", position: "relative", overflow: "hidden" }}>
          ...
          {timeline.map((item, i) => (
            <div key={i} style={{ display: "flex", gap: "1.25rem", paddingBottom: i < timeline.length - 1 ? "1.5rem" : 0, marginBottom: i < timeline.length - 1 ? "1.5rem" : 0, borderBottom: i < timeline.length - 1 ? "1px solid rgba(108,99,255,0.08)" : "none" }}>
              {/* Dot + line */}
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "var(--font-mono, monospace)", fontSize: "0.7rem", fontWeight: 700, color: "#6c63ff", marginBottom: "0.25rem", letterSpacing: "0.06em" }}>{item.year}</div>
                <div style={{ fontSize: "0.9rem", color: "#5b5478", fontWeight: 300, lineHeight: 1.6 }}>{item.text}</div>
              </div>
            </div>
          ))}

          {/* "Just getting started" note */}
          <div style={{ marginTop: "1.5rem", paddingTop: "1.25rem", borderTop: "1px solid rgba(108,99,255,0.08)", display: "flex", alignItems: "center", gap: "0.6rem" }}>
