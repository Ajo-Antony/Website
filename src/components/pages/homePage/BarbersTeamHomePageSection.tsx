...
const GRADS = ["linear-gradient(135deg,var(--accent),var(--accent-2))", "linear-gradient(135deg,var(--accent-teal),#38bdf8)", "linear-gradient(135deg,#f472b6,#fb7185)", "linear-gradient(135deg,#f59e0b,#fbbf24)"];

export default function BarbersTeamHomePageSection({ eyebrow = D.eyebrow, heading = D.heading, members = D.members }: TeamProps) {
  return (
    <section id="team" style={{ padding: "8rem 0", borderTop: "1px solid var(--glass-bg)" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 1.5rem" }}>
        <div style={{ marginBottom: "3.5rem" }}>
          <div style={{ display: "inline-flex", fontFamily: "var(--font-mono, monospace)", fontSize: "0.65rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--accent-deep)", background: "rgba(108,99,255,0.07)", border: "1px solid var(--glass-bg)", padding: "0.3rem 0.8rem", borderRadius: 100, marginBottom: "1.25rem" }}>{eyebrow}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: "1.5rem" }}>
          {members.map((member, i) => (
            <div key={member.name} style={{ padding: "2rem", borderRadius: 22, background: "rgba(255,255,255,0.55)", border: "1px solid rgba(255,255,255,0.85)", backdropFilter: "blur(20px)", boxShadow: "0 8px 32px rgba(99,88,210,0.10)", textAlign: "center", transition: "transform 0.25s" }}>
              ...
              <div style={{ fontFamily: "var(--font-mono, monospace)", fontSize: "0.9rem", fontWeight: 700, color: "var(--text)", marginBottom: "0.25rem" }}>{member.name}</div>
              <div style={{ fontFamily: "var(--font-mono, monospace)", fontSize: "0.68rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--accent-deep)", marginBottom: "0.75rem" }}>{member.role}</div>
              <p style={{ fontSize: "0.82rem", color: "var(--text-muted)", fontWeight: 300, lineHeight: 1.65 }}>{member.bio}</p>
            </div>
          ))}
        </div>
      </div>
