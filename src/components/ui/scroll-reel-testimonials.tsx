...
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    ...
    >
      ...
      />
      {/* diagonal gradient sheen — brand accent → teal */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 z-[3] blur-[6px] mix-blend-overlay"
        style={{
          background:
            "linear-gradient(220.99deg, rgba(108,99,255,0) 32%, rgb(108,99,255) 41%, rgb(167,139,250) 47%, rgba(14,165,233,0.57) 54%, rgba(14,165,233,0) 65%)",
        }}
      />
    </div>
  );
