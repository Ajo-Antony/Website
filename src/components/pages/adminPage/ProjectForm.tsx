...
export default function ProjectForm({
  ...
  return (
    <form action={handleSubmit} className="bg-[var(--surface)] rounded-2xl border border-gray-200 p-8 max-w-2xl">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
          />
        </div>
        <div>
      ...
      <label className="block text-sm font-medium text-gray-700 mb-1">
        Summary <span className="text-gray-400">(shown on the project card)</span>
      </label>
      <textarea
        name="summary"
      ...
      <label className="block text-sm font-medium text-gray-700 mb-1">
        Cover image {project?.cover_image && <span className="text-gray-400">(leave blank to keep current)</span>}
      </label>
      {project?.cover_image && (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={project.cover_image} alt="" className="w-32 h-20 object-cover rounded-lg mb-2 border border-gray-200" />
      )}
      <input
        type="file"
        name="cover_image"
      ...
      <label className="block text-sm font-medium text-gray-700 mb-1">
        Full description <span className="text-gray-400">(Markdown supported)</span>
      </label>
      <textarea
        name="description"
        ...
        className="w-full mb-4 px-4 py-2.5 border border-gray-200 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
      />
      ...
      <label className="block text-sm font-medium text-gray-700 mb-1">
        Results / metrics <span className="text-gray-400">(one "Label: Value" per line, optional)</span>
      </label>
      <textarea
        name="results"
      ...
      <label className="block text-sm font-medium text-gray-700 mb-1">
        Live link <span className="text-gray-400">(optional)</span>
      </label>
      <input
        name="link"
        defaultValue={project?.link ?? ""}
        placeholder="https://example.com"
        className="w-full mb-5 px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
      />
