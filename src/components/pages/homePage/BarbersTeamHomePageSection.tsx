          ...
          {members.map((member, i) => (
            <div key={member.name} style={{ padding: "2rem", borderRadius: 22, background: "rgba(255,255,255,0.55)", border: "1px solid rgba(255,255,255,0.85)", backdropFilter: "blur(20px)", boxShadow: "0 8px 32px rgba(99,88,210,0.10)", textAlign: "center", transition: "transform 0.25s" }}>
              {member.photo && member.photo.trim() ? (
                ...
                <div style={{ width: 72, height: 72, borderRadius: "50%", background: GRADS[i % GRADS.length], display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1.25rem", fontFamily: "var(--font-body)", fontSize: "1.1rem", fontWeight: 700, color: "#fff" }}>{member.initials}</div>
              )}
              <div style={{ fontFamily: "var(--font-body)", fontSize: "0.9rem", fontWeight: 700, color: "var(--text)", marginBottom: "0.25rem" }}>{member.name}</div>
              <div style={{ fontFamily: "var(--font-body)", fontSize: "0.68rem", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--accent-deep)", marginBottom: "0.75rem" }}>{member.role}</div>
