...
const PANEL_TABS = [
  { key: "bg",      label: "Background", Icon: IconPaintBucket },
  { key: "text",    label: "Typography", Icon: IconTypeAa },
  { key: "layout",  label: "Layout",     Icon: IconLayoutGrid },
  { key: "marquee", label: "Marquee",    Icon: IconInfinity },
  { key: "slider",  label: "Slider",     Icon: IconCarousel },
  { key: "images",  label: "Images",     Icon: IconImageLandscape },
];
...
export default function DesignPanel({ section, onSave, onClose }: Props) {
  ...
  function ColorPicker({ label, value, onChange }: { label: string; value?: string; onChange: (v: string) => void }) {
    return (
      <div>
        <label className="block text-xs text-ink-soft font-medium mb-1.5">{label}</label>
        <div className="flex items-center gap-2">
          <input
            type="color"
            value={value || "#ffffff"}
            onChange={(e) => onChange(e.target.value)}
            className="w-9 h-9 rounded-lg border border-line cursor-pointer p-0.5"
          />
          <input
            type="text"
            value={value || ""}
  ...
  function renderBgTab() {
    ...
    return (
      <div className="space-y-5">
        {/* Type selector */}
        <div>
          <label className="block text-xs text-ink-soft font-medium mb-2">Background type</label>
        ...
        {bgType === "gradient" && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs text-ink-soft font-medium mb-2">Gradient presets</label>
              <div className="grid grid-cols-4 gap-2">
                {GRADIENT_PRESETS.map((p) => (
                  <button
                    ...
                    className="h-9 rounded-lg border-2 border-transparent hover:border-accent/50 transition-all"
                  />
                ))}
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs text-ink-soft font-medium mb-1.5">Direction</label>
              <select
                value={design.bgGradientDir ?? "to bottom"}
                onChange={(e) => set("bgGradientDir", e.target.value as any)}
                className="w-full text-xs border border-line rounded-lg px-2.5 py-2 focus:outline-none focus:ring-1 focus:ring-accent"
              >
        ...
        {bgType === "pattern" && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs text-ink-soft font-medium mb-2">Pattern style</label>
              <div className="grid grid-cols-4 gap-2">
                {PATTERNS.map((p) => (
                  <button
                    key={p.key}
        ...
        {bgType === "image" && (
          <div className="space-y-4">
            <div>
              <label className="block text-xs text-ink-soft font-medium mb-1.5">Image URL</label>
              <input
                type="url"
            ...
            <div>
              <label className="block text-xs text-ink-soft font-medium mb-1.5">Or upload image</label>
              <label className="flex items-center justify-center gap-2 cursor-pointer px-3 py-2.5 bg-accent/5 border border-dashed border-accent/30 rounded-lg text-xs text-accent font-semibold hover:bg-accent/10 transition-colors">
                <IconUploadCloud size={14} strokeWidth={1.8} />
                Choose file
        ...
        {/* Live preview */}
        <div>
          <label className="block text-xs text-ink-soft font-medium mb-1.5">Preview</label>
          <div
            className="h-20 rounded-xl border border-line relative overflow-hidden"
  ...
  function renderTextTab() {
    const ALIGN_OPTS = [
      { key: "left" as const,   label: "Left",   Icon: IconAlignLeft },
      { key: "center" as const, label: "Center", Icon: IconAlignCenter },
      { key: "right" as const,  label: "Right",  Icon: IconAlignRight },
    ];
    return (
      <div className="space-y-5">
        ...
        <div>
          <label className="block text-xs text-ink-soft font-medium mb-2">Text alignment</label>
          <div className="flex gap-2">
            {ALIGN_OPTS.map((a) => (
              <button key={a.key}
                onClick={() => set("textAlign", a.key)}
  ...
  function renderLayoutTab() {
    return (
      <div className="space-y-5">
        <div>
          <label className="block text-xs text-ink-soft font-medium mb-2">Section width</label>
          <div className="grid grid-cols-4 gap-2">
        ...
        <div className="flex flex-col gap-3">
          <label className="flex items-center gap-2.5 cursor-pointer">
            ...
            />
            <span className="text-xs font-medium text-ink">Top border</span>
          </label>
          <label className="flex items-center gap-2.5 cursor-pointer">
            <input type="checkbox" checked={design.borderBottom ?? false}
              ...
              className="w-4 h-4 rounded accent-accent"
            />
            <span className="text-xs font-medium text-ink">Bottom border</span>
          </label>
          <label className="flex items-center gap-2.5 cursor-pointer">
            <input type="checkbox" checked={design.roundedCorners ?? false}
              ...
              className="w-4 h-4 rounded accent-accent"
            />
            <span className="text-xs font-medium text-ink">Rounded corners (card style)</span>
          </label>
        </div>
        ...
        <div>
          <label className="block text-xs text-ink-soft font-medium mb-1.5">Custom CSS class</label>
          <input type="text"
            value={design.customClass ?? ""}
            onChange={(e) => set("customClass", e.target.value)}
            placeholder="e.g. my-special-section"
            className="w-full text-xs font-mono border border-line rounded-lg px-2.5 py-2 focus:outline-none focus:ring-1 focus:ring-accent"
          />
        </div>
      </div>
    );
  ...
  function renderMarqueeTab() {
    return (
      <div className="space-y-5">
        <label className="flex items-center gap-3 cursor-pointer p-3 bg-accent/5 rounded-xl border border-accent/10">
          ...
          <div>
            ...
            <div className="text-xs text-ink-dim">Content scrolls horizontally in an infinite loop</div>
          </div>
        </label>
        ...
        {design.enableMarquee && (
          <>
            <div>
              <label className="block text-xs text-ink-soft font-medium mb-2">Speed</label>
              <div className="flex gap-2">
            ...
            <div>
              <label className="block text-xs text-ink-soft font-medium mb-2">Direction</label>
              <div className="flex gap-2">
                {([
                  { key: "left" as const,  label: "Left",  Icon: IconArrowLeft },
                  { key: "right" as const, label: "Right", Icon: IconArrowRight },
                ]).map((d) => (
                  <button key={d.key} onClick={() => set("marqueeDirection", d.key)}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs rounded-lg font-semibold border transition-all ${
                    ...
                    {d.label}
                    {d.key === "right" && <d.Icon size={13} strokeWidth={1.8} />}
                  </button>
                ))}
              </div>
            </div>
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" checked={design.marqueePauseHover ?? true}
                ...
                className="w-4 h-4 rounded accent-accent"
              />
              <span className="text-xs font-medium text-ink">Pause on hover</span>
            </label>
          </>
        )}
      </div>
      ...
      <div className="space-y-5">
        <label className="flex items-center gap-3 cursor-pointer p-3 bg-accent/5 rounded-xl border border-accent/10">
          ...
          <div>
            ...
            <div className="text-xs text-ink-dim">Items shown in a scrollable carousel</div>
          </div>
        </label>
        ...
        {design.enableSlider && (
          <>
            <div>
              <label className="block text-xs text-ink-soft font-medium mb-2">Transition style</label>
              <div className="flex gap-2">
            ...
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input type="checkbox" checked={design.sliderAutoplay ?? true}
                ...
                className="w-4 h-4 rounded accent-accent"
              />
              <span className="text-xs font-medium text-ink">Autoplay</span>
            </label>
  ...
  function renderImagesTab() {
    return (
      <div className="space-y-4">
        <p className="text-xs text-ink-soft bg-accent/5 rounded-xl px-3 py-2 border border-accent/10">
        ...
        {/* Upload */}
        <div>
          <label className="flex items-center justify-center gap-2 cursor-pointer px-3 py-3 bg-accent/5 border-2 border-dashed border-accent/25 rounded-xl text-xs text-accent font-semibold hover:bg-accent/10 transition-colors">
            {uploading ? (
              <>
                <IconSpinner size={14} />
                Uploading…
        ...
        {(design.images ?? []).length > 0 ? (
          <div className="space-y-2">
            {(design.images ?? []).map((img) => (
              <div key={img.id} className="flex items-start gap-3 p-3 bg-surface-alt/60 rounded-xl border border-line">
                <img src={img.url} alt={img.alt ?? ""} className="w-14 h-14 rounded-lg object-cover border border-line shrink-0" />
                <div className="flex-1 min-w-0 space-y-1.5">
                  <input
                    ...
                    className="w-full text-xs border border-line rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-accent"
                  />
                  <select
                ...
                <button onClick={() => removeImage(img.id)} className="text-ink-dim hover:text-rose-500 shrink-0 mt-0.5 transition-colors">
          ...
          <div className="text-center py-8 text-ink-dim/60 text-sm">No images added yet</div>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-line overflow-hidden flex flex-col" style={{ maxHeight: "85vh" }}>
      ...
      {/* Icon tab rail */}
      <div className="flex items-center gap-1 px-3 py-2 bg-surface-alt/60 border-b border-line/70 shrink-0">
        {PANEL_TABS.map((t) => {
      ...
      <div className="px-5 py-3 border-t border-line/70 flex gap-2 shrink-0 bg-surface-alt/60">
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex-1 flex items-center justify-center gap-1.5 text-xs px-3 py-2 rounded-lg bg-accent text-white font-bold transition-all hover:bg-accent-deep disabled:opacity-60"
        >
          {saving ? (
            <>
              <IconSpinner size={13} />
