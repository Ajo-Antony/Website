        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: "1.5rem" }}>
          {members.map((m, i) => (
            <div key={m.name} style={{ background: "var(--surface-alt)", borderRadius: 22, padding: "2rem", border: "1px solid var(--border)", textAlign: "center", transition: "transform 0.3s ease, box-shadow 0.3s ease" }}>
              ...
              <div style={{ fontSize: "1rem", fontWeight: 700, color: "var(--text)", marginBottom: "0.25rem", letterSpacing: "-0.01em" }}>{m.name}</div>
              <div style={{ fontSize: "0.75rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" as const, color: "var(--accent)", marginBottom: "0.75rem" }}>{m.role}</div>
              <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", lineHeight: 1.65 }}>{m.bio}</p>
            </div>
          ))}
        </div>
      </div>
