    ...
    <section style={{ padding: "6rem 0", background: "#F4F2FE", borderTop: "1px solid var(--border)" }}>
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 2rem" }}>
        <div style={{ borderRadius: 32, padding: "4.5rem 3rem", textAlign: "center", background: "var(--surface)", border: "1px solid var(--border)", boxShadow: "0 24px 80px var(--border)", position: "relative", overflow: "hidden" }}>
          ...
          <h2 style={{ fontSize: "clamp(1.9rem,4vw,3rem)", fontWeight: 800, letterSpacing: "-0.03em", color: "var(--text)", marginBottom: "1rem" }}>{heading}</h2>
          <p style={{ fontSize: "1rem", color: "var(--text-muted)", marginBottom: "2.5rem", fontWeight: 400 }}>{paragraph}</p>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem", flexWrap: "wrap" as const }}>
            <Link href={primaryCtaHref} style={{ fontSize: "1rem", fontWeight: 700, padding: "0.9rem 2.25rem", background: "linear-gradient(135deg,var(--accent),var(--accent-2))", color: "#fff", borderRadius: 100, textDecoration: "none", boxShadow: "0 8px 24px var(--shadow-strong)" }}>{primaryCtaLabel}</Link>
            <Link href={secondaryCtaHref} style={{ fontSize: "1rem", fontWeight: 600, padding: "0.9rem 2.25rem", background: "#F4F2FE", color: "var(--accent-deep)", borderRadius: 100, textDecoration: "none", border: "1.5px solid rgba(108,99,255,0.2)" }}>{secondaryCtaLabel}</Link>
          </div>
        </div>
      </div>
    </section>
